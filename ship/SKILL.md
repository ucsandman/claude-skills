...
11. **Commit and push.** Message follows repo convention. Push, then confirm CI kicked off (and passes, if it's fast). Memory-dir changes get their own commit in `~/.claude`. In agnostic-ai, also `git push claude-harness` (the starred public mirror; `npm run doctor` fails on `mirror-current` if it is behind). A skill, hook or rule change ends with `npm run port` from agnostic-ai: the doctor's `rules-drift` check compares the ported copies in every client home and stays red until they are rewritten.
